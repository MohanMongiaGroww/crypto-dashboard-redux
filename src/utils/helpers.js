...
export function checkIfLocalStorageDataExpired(
  coinKey,
  expiryKey,
  uuid = null
) {
  ...
  if (coinKey === "coin") {
    ...
    ifDataValid =
      coin?.uuid === uuid && Expiry && Expiry > Number.parseInt(Date.now());
...
export function getCoinSuggestion(value, coins) {
  ...
  return coins.filter((coin) => {
    ...
    if (coinName.length >= valueLength) {
      if (coinName.substr(0, valueLength) === value) return true;
    }
    if (coinSymbol.length >= valueLength) {
      if (coinSymbol.substr(0, valueLength) === value) return true;
    }
    return false;
  });
}
